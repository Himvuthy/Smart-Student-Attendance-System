...
const StudentDashboard = ({ onLogout }) => {
  ...
  const AttendanceTable = ({ compact = false }) => (
    <div className="overflow-x-auto">
      <table className={`w-full text-left text-sm ${compact ? 'min-w-[680px]' : 'min-w-[820px]'}`}>
        <thead>
          <tr className="border-b border-[#e2e8f0] bg-slate-100/80 text-xs uppercase tracking-wider text-slate-700 dark:border-white/10 dark:bg-white/[0.05] dark:text-slate-300">
  ...
  const renderOverview = () => {
    ...
    <div className="space-y-4">
        ...
        {[
          ...
          <div key={label} className={`${card} flex min-h-44 flex-col p-5`}>
            <div className="flex items-center gap-3">
              <div className="rounded-full border border-[#e2e8f0] p-2 text-slate-600 dark:border-white/10 dark:text-slate-300">{React.createElement(icon, { size: 18 })}</div>
            ...
            <div className="mt-auto pt-3">
              {progress ? (
                ...
                <button onClick={() => label === 'Next Class' ? setActiveView('schedule') : setActiveView('attendance')} className="flex w-full items-center justify-between rounded-lg border border-[#e2e8f0] px-3 py-2 text-xs font-bold transition hover:bg-slate-50 dark:border-white/10 dark:hover:bg-white/5"><span>{note}</span><ChevronRight size={15} /></button>
              )}
            </div>
          </div>
        ))}
      ...
      <section className="grid gap-4 xl:grid-cols-[1.65fr_1fr]">
        <div className={`${card} overflow-hidden`}>
          <div className="flex items-center justify-between border-b border-[#e2e8f0] px-5 py-4 dark:border-white/10">
            ...
            <div className="flex rounded-xl border border-[#e2e8f0] p-1 text-xs font-bold dark:border-white/10"><span className="rounded-lg bg-white px-3 py-1.5 shadow-sm dark:bg-white/10">Weekly</span><span className={`px-3 py-1.5 ${muted}`}>Monthly</span></div>
          </div>
          <div className="p-5">
        ...
        <div className="space-y-4">
          <div className={`${card} overflow-hidden`}>
            <div className="flex items-center justify-between border-b border-[#e2e8f0] px-5 py-4 dark:border-white/10"><h3 className="font-extrabold">Today’s Classes</h3><span className={`flex items-center gap-2 text-xs ${muted}`}><span className="h-2 w-2 rounded-full bg-emerald-400" />{todaysClasses.length} sessions</span></div>
          ...
          <div className={`${card} overflow-hidden`}>
            <div className="flex items-center justify-between border-b border-[#e2e8f0] px-5 py-4 dark:border-white/10"><h3 className="font-extrabold">Attendance Breakdown</h3><Fingerprint size={18} className={muted} /></div>
      ...
      <section className={`${card} overflow-hidden`}>
        <div className="flex items-center justify-between border-b border-[#e2e8f0] px-5 py-4 dark:border-white/10"><div><h3 className="font-extrabold">Recent Attendance</h3><p className={`mt-0.5 text-xs ${muted}`}>Latest fingerprint check-ins</p></div><button onClick={() => setActiveView('attendance')} className="rounded-lg border border-[#e2e8f0] p-2 text-slate-500 dark:border-white/10"><Search size={17} /></button></div>
  ...
  const renderAttendance = () => (
    <div className="space-y-6">
      ...
      <div className={`${card} overflow-hidden`}>
        <div className="flex flex-col gap-3 border-b border-[#e2e8f0] p-5 dark:border-white/10 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            <label className="relative">
              ...
              <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search class..." className="w-full rounded-xl border border-[#e2e8f0] bg-slate-50 py-2.5 pl-9 pr-3 text-sm outline-none focus:border-sky-400 dark:border-white/10 dark:bg-white/5 sm:w-52" />
            </label>
            <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="rounded-xl border border-[#e2e8f0] bg-slate-50 px-3 py-2.5 text-sm font-medium outline-none focus:border-sky-400 dark:border-white/10 dark:bg-[#182133]">
      ...
      {correctionRequests.length > 0 && <section className={`${card} overflow-hidden`}><div className="border-b border-[#e2e8f0] p-5 dark:border-white/10"><h3 className="font-extrabold">Correction requests</h3><p className={`mt-1 text-xs ${muted}`}>Track requests submitted for incorrectly recorded attendance.</p></div><div className="divide-y divide-[#e2e8f0] dark:divide-white/5">{correctionRequests.map((request) => <div key={request.id} className="flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-bold">{request.course} · {request.date}</p><p className={`mt-1 text-xs ${muted}`}>Recorded {request.recordedStatus} → requested {request.expectedStatus}</p></div><span className={`w-fit rounded-full px-2.5 py-1 text-xs font-bold ${request.status === 'Approved' ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-400/10 dark:text-emerald-300' : request.status === 'Rejected' ? 'bg-rose-50 text-rose-700 dark:bg-rose-400/10 dark:text-rose-300' : 'bg-amber-50 text-amber-700 dark:bg-amber-400/10 dark:text-amber-300'}`}>{request.status}</span></div>)}</div></section>}
    </div>
  );
  ...
  const renderSchedule = () => (
    <div className="space-y-4">
      <section className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          <button onClick={() => setCalendarMonth((value) => (value + 11) % 12)} className="rounded-xl border border-[#e2e8f0] bg-white p-2.5 hover:border-sky-300 hover:text-sky-600 dark:border-white/10 dark:bg-white/5"><ChevronLeft size={17} /></button>
          ...
          <button onClick={() => setCalendarMonth((value) => (value + 1) % 12)} className="rounded-xl border border-[#e2e8f0] bg-white p-2.5 hover:border-sky-300 hover:text-sky-600 dark:border-white/10 dark:bg-white/5"><ChevronRight size={17} /></button>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button onClick={() => { setCalendarMonth(6); setSelectedDay(28); setCalendarMode('week'); }} className="rounded-xl border border-[#e2e8f0] bg-white px-3 py-2 text-xs font-bold hover:border-sky-300 dark:border-white/10 dark:bg-white/5">Today</button>
      ...
      <div className={`${card} min-h-[calc(100vh-150px)] w-full overflow-hidden lg:grid lg:grid-cols-[230px_minmax(0,1fr)]`}>
        <aside className="border-b border-[#e2e8f0] bg-slate-50/50 p-4 dark:border-white/10 dark:bg-white/[0.02] lg:border-b-0 lg:border-r">
          ...
          <div className="mt-5 border-t border-[#e2e8f0] pt-4 dark:border-white/10">
          ...
          <div className="mt-5 grid grid-cols-2 gap-2 border-t border-[#e2e8f0] pt-4 dark:border-white/10">
        ...
        <div className="overflow-x-auto">
          {calendarMode === 'month' ? (
            <div className="min-w-[760px] p-5">
              <div className="grid grid-cols-7 border-l border-t border-[#e2e8f0] dark:border-white/10">
                {['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'].map((day) => <div key={day} className={`border-b border-r border-[#e2e8f0] p-3 text-center text-xs font-bold dark:border-white/10 ${muted}`}>{day}</div>)}
                {[...Array(35)].map((_, i) => {
                  const day = i + 1;
                  const hasClass = [1,3,7,8,10,14,15,17,21,22,24,28,29,31].includes(day);
                  return <button key={i} onClick={() => setSelectedDay(day)} className={`min-h-24 border-b border-r border-[#e2e8f0] p-2 text-left align-top transition hover:bg-sky-50 dark:border-white/10 dark:hover:bg-sky-400/5 ${selectedDay === day ? 'bg-sky-50 ring-2 ring-inset ring-[#60a5fa] dark:bg-sky-400/10' : ''}`}><span className="text-xs font-bold">{day <= 31 ? day : ''}</span>{day <= 31 && hasClass && <div className="mt-3 rounded-md bg-sky-100 p-2 text-[10px] font-bold text-sky-700 dark:bg-sky-400/15 dark:text-sky-300">Class scheduled</div>}</button>;
                })}
              </div>
            </div>
          ) : (
          <div className="min-w-[850px]">
            <div className="grid grid-cols-[64px_repeat(5,1fr)] border-b border-[#e2e8f0] dark:border-white/10">
              <div className={`p-3 text-center text-[10px] ${muted}`}>UTC+7</div>
              {[['28','Monday'],['29','Tuesday'],['30','Wednesday'],['31','Thursday'],['01','Friday']].map(([date, day]) => <div key={day} className={`border-l border-[#e2e8f0] p-3 text-center dark:border-white/10 ${day === 'Tuesday' ? 'bg-sky-50/50 dark:bg-sky-400/5' : ''}`}><p className="text-sm font-black">{date}</p><p className={`text-[10px] ${muted}`}>{day}</p></div>)}
            </div>
            <div className="relative grid min-h-[640px] grid-cols-[64px_repeat(5,1fr)] grid-rows-[repeat(8,minmax(76px,1fr))]">
              {[8,9,10,11,12,13,14,15].map((hour, row) => <React.Fragment key={hour}><div style={{ gridColumn: 1, gridRow: row + 1 }} className={`border-b border-[#e2e8f0] px-2 pt-3 text-[10px] dark:border-white/10 ${muted}`}>{String(hour > 12 ? hour - 12 : hour).padStart(2,'0')} {hour >= 12 ? 'PM' : 'AM'}</div>{[1,2,3,4,5].map((day) => <div key={day} style={{ gridColumn: day + 1, gridRow: row + 1 }} className="border-b border-l border-[#e2e8f0] dark:border-white/10" />)}</React.Fragment>)}
      ...
      {reminderOpen && <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/45 p-4 backdrop-blur-sm" onClick={() => setReminderOpen(false)}><form onClick={(event) => event.stopPropagation()} onSubmit={(event) => { event.preventDefault(); const updatedItem = { day: Number(reminderDraft.day), row: Math.max(1, Number(reminderDraft.hour) - 7), span: editingEventId ? (calendarItems.find((item) => item.id === editingEventId)?.span || 1) : 1, title: reminderDraft.title, time: `${String(reminderDraft.hour).padStart(2,'0')}:00 reminder`, room: reminderDraft.room, tone: editingEventId ? (calendarItems.find((item) => item.id === editingEventId)?.tone || 'bg-sky-100 border-sky-400 dark:bg-sky-400/15') : 'bg-sky-100 border-sky-400 dark:bg-sky-400/15' }; setCalendarItems((items) => editingEventId ? items.map((item) => item.id === editingEventId ? { ...item, ...updatedItem } : item) : [...items, { ...updatedItem, id: `item-${Date.now()}` }]); setVisibleClasses((value) => ({ ...value, [reminderDraft.title]: true })); setReminderOpen(false); setEditingEventId(null); setReminderDraft({ title: '', day: 1, hour: 8, room: '' }); }} className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl dark:bg-[#151d2c]"><div className="flex items-center justify-between"><h3 className="text-xl font-black">{editingEventId ? 'Edit calendar item' : 'Add reminder'}</h3><button type="button" onClick={() => { setReminderOpen(false); setEditingEventId(null); }} className="rounded-lg p-2 hover:bg-slate-100 dark:hover:bg-white/5"><X size={18} /></button></div><div className="mt-5 space-y-4"><label className="block text-xs font-bold">Title<input required value={reminderDraft.title} onChange={(e) => setReminderDraft((value) => ({ ...value, title: e.target.value }))} className="mt-2 w-full rounded-xl border border-[#e2e8f0] bg-transparent p-3 text-sm outline-none focus:border-[#60a5fa] dark:border-white/10" placeholder="Study group" /></label><div className="grid grid-cols-2 gap-3"><label className="text-xs font-bold">Day<select value={reminderDraft.day} onChange={(e) => setReminderDraft((value) => ({ ...value, day: e.target.value }))} className="mt-2 w-full rounded-xl border border-[#e2e8f0] bg-white p-3 text-sm dark:border-white/10 dark:bg-[#151d2c]">{['Monday','Tuesday','Wednesday','Thursday','Friday'].map((day, i) => <option key={day} value={i + 1}>{day}</option>)}</select></label><label className="text-xs font-bold">Start time<select value={reminderDraft.hour} onChange={(e) => setReminderDraft((value) => ({ ...value, hour: e.target.value }))} className="mt-2 w-full rounded-xl border border-[#e2e8f0] bg-white p-3 text-sm dark:border-white/10 dark:bg-[#151d2c]">{[8,9,10,11,12,13,14,15].map((hour) => <option key={hour} value={hour}>{hour > 12 ? hour - 12 : hour}:00 {hour >= 12 ? 'PM' : 'AM'}</option>)}</select></label></div><label className="block text-xs font-bold">Room or note<input value={reminderDraft.room} onChange={(e) => setReminderDraft((value) => ({ ...value, room: e.target.value }))} className="mt-2 w-full rounded-xl border border-[#e2e8f0] bg-transparent p-3 text-sm outline-none focus:border-[#60a5fa] dark:border-white/10" placeholder="Library" /></label></div><button type="submit" className="mt-6 w-full rounded-xl bg-[#60a5fa] py-3 text-sm font-bold text-white hover:bg-[#3b82f6]">{editingEventId ? 'Save changes' : 'Save reminder'}</button></form></div>}
    </div>
  );
      ...
      <div className="grid items-start gap-5 xl:grid-cols-[280px_minmax(0,1fr)]">
        <aside className={`${card} overflow-hidden xl:sticky xl:top-5`}>
          ...
          <div className="px-5 pb-5">
            ...
            <div className="mt-4 flex flex-wrap gap-2"><button onClick={() => profilePhotoInputRef.current?.click()} className="rounded-lg bg-sky-50 px-3 py-2 text-[10px] font-bold text-sky-700 transition hover:bg-sky-100 dark:bg-sky-400/10 dark:text-sky-300">Choose photo</button>{profilePhoto && <button onClick={() => setProfilePhotoEditorSource(profilePhoto)} className="rounded-lg border border-[#e2e8f0] px-3 py-2 text-[10px] font-bold text-slate-600 transition hover:border-sky-300 hover:text-sky-600 dark:border-white/10 dark:text-slate-300">Edit crop</button>}{profilePhoto && <button onClick={removeProfilePhoto} className="rounded-lg border border-[#e2e8f0] px-3 py-2 text-[10px] font-bold text-slate-500 transition hover:border-rose-300 hover:text-rose-600 dark:border-white/10 dark:text-slate-400">Remove</button>}</div>
            ...
            <div className="mt-5 rounded-xl bg-slate-50 p-3 dark:bg-white/5">
              ...
              <div className="mt-1 flex items-center justify-between gap-2"><b className="truncate text-xs">{student.id}</b><button onClick={() => navigator.clipboard?.writeText(student.id)} className="rounded-lg border border-[#e2e8f0] px-2 py-1 text-[10px] font-bold transition hover:border-violet-400 hover:text-violet-600 dark:border-white/10">Copy</button></div>
            </div>
            <div className="mt-5 space-y-2 border-t border-[#e2e8f0] pt-4 dark:border-white/10">
        ...
        <div className="min-w-0 space-y-5">
          <div className="flex gap-6 border-b border-[#e2e8f0] dark:border-white/10" role="tablist" aria-label="Student profile sections">
          ...
          {(profileTab === 'personal' ? [
            ...
            <section key={title} className={`${card} overflow-hidden`}>
              <div className="flex items-center justify-between border-b border-[#e2e8f0] px-5 py-4 dark:border-white/10"><h3 className="text-sm font-extrabold">{title}</h3><span className={`rounded-full px-2.5 py-1 text-[10px] font-bold ${status === 'Verified' || status === 'Enrolled' || status === 'Active' ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-400/10 dark:text-emerald-300' : muted}`}>{status}</span></div>
            ...
            <article key={course.code} className={`${card} group overflow-hidden shadow-[0_10px_28px_rgba(39,55,105,0.11)] transition duration-300 hover:-translate-y-0.5 hover:shadow-[0_16px_36px_rgba(73,85,160,0.17)] dark:shadow-[0_12px_30px_rgba(0,0,0,0.28)]`}>
              <div className="p-5">
                ...
                <div className="mt-5 flex gap-2">
                  ...
                  <button onClick={() => setActiveView('attendanceReport')} aria-label={`Open attendance report for ${course.name}`} className="rounded-xl border border-[#e2e8f0] px-3 text-violet-600 transition hover:border-violet-300 dark:border-white/10"><FileCheck2 size={16} /></button>
                </div>
              </div>
            </article>
        ))}
    ...
    <div className="grid gap-6 xl:grid-cols-[1fr_1.1fr]">
      <form onSubmit={submitExcuseRequest} className={`${card} p-6`}>
        ...
        <div className="mt-6 space-y-4">
          <label className="block text-xs font-bold">Absence date<input type="date" required value={excuseDraft.date} onChange={(e) => setExcuseDraft((value) => ({ ...value, date: e.target.value }))} className="mt-2 w-full rounded-xl border border-[#e2e8f0] bg-transparent p-3 text-sm dark:border-white/10" /></label>
          <label className="block text-xs font-bold">Course<select value={excuseDraft.course} onChange={(e) => setExcuseDraft((value) => ({ ...value, course: e.target.value }))} className="mt-2 w-full rounded-xl border border-[#e2e8f0] bg-white p-3 text-sm dark:border-white/10 dark:bg-[#151d2c]">{displayCourseStats.map((course) => <option key={course.code} value={course.code}>{course.code} — {course.name}</option>)}</select></label>
          <label className="block text-xs font-bold">Reason<select value={excuseDraft.reason} onChange={(e) => setExcuseDraft((value) => ({ ...value, reason: e.target.value }))} className="mt-2 w-full rounded-xl border border-[#e2e8f0] bg-white p-3 text-sm dark:border-white/10 dark:bg-[#151d2c]">{['Medical','Family emergency','University activity','Transportation','Other'].map((reason) => <option key={reason}>{reason}</option>)}</select></label>
          <label className="block text-xs font-bold">Explanation<textarea required rows="4" value={excuseDraft.details} onChange={(e) => setExcuseDraft((value) => ({ ...value, details: e.target.value }))} className="mt-2 w-full resize-none rounded-xl border border-[#e2e8f0] bg-transparent p-3 text-sm dark:border-white/10" placeholder="Explain your absence..." /></label>
      ...
      <section className={`${card} overflow-hidden`}>
        <div className="border-b border-[#e2e8f0] p-5 dark:border-white/10"><h3 className="font-extrabold">My requests</h3><p className={`mt-1 text-xs ${muted}`}>Track submitted absence explanations.</p></div>
  ...
  const renderAttendanceReport = () => (
    <div className="space-y-6">
      ...
      <section>
        <div className="flex border-b border-[#e2e8f0] dark:border-white/10" role="tablist" aria-label="Attendance report views">
        ...
        <div className={`${card} overflow-hidden`}>
          <div className="flex flex-col gap-3 border-b border-[#e2e8f0] p-4 sm:flex-row sm:items-center sm:justify-between dark:border-white/10">
            <label className="relative block w-full sm:max-w-sm">
              ...
              <input
                ...
                className="w-full rounded-xl border border-[#e2e8f0] bg-white py-2.5 pl-10 pr-4 text-sm outline-none transition focus:border-[#60a5fa] focus:ring-2 focus:ring-sky-200/60 dark:border-white/10 dark:bg-white/5"
              />
            </label>
            <select
              ...
              className="rounded-xl border border-[#e2e8f0] bg-white px-3 py-2.5 text-sm font-semibold outline-none transition focus:border-[#60a5fa] focus:ring-2 focus:ring-sky-200/60 dark:border-white/10 dark:bg-[#151d2c]"
            >
              <option>All</option>
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[720px] text-left text-sm">
              <thead className="border-b border-[#e2e8f0] bg-slate-100/80 text-xs font-semibold text-slate-700 dark:border-white/10 dark:bg-white/[0.05] dark:text-slate-300">
                <tr>{[
          ...
          <div className="flex flex-col gap-3 border-t border-[#e2e8f0] px-5 py-4 sm:flex-row sm:items-center sm:justify-between dark:border-white/10">
  ...
  const renderSettings = () => {
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid gap-5 xl:grid-cols-[1.35fr_0.85fr]">
          ...
          <div className="space-y-5">
            <section className={`${card} p-5`}>
              ...
              <button onClick={() => setActiveView('profile')} className="mt-3 w-full rounded-xl border border-[#e2e8f0] py-2.5 text-xs font-bold transition hover:border-[#60a5fa] hover:text-[#2563eb] dark:border-white/10">View student profile</button>
            </section>
        ...
        <section className={`${card} overflow-hidden`}>
          <div className="border-b border-[#e2e8f0] p-5 dark:border-white/10"><h3 className="font-extrabold">My preferences</h3><p className={`mt-1 text-xs ${muted}`}>Personalize how the student portal works for you.</p></div>
          <div className="divide-y divide-[#e2e8f0] px-5 dark:divide-white/10">
            <div className="flex flex-col gap-4 py-5 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex gap-2">
                {[{ dark: false, label: 'Daylight', icon: Sun }, { dark: true, label: 'Soft sky', icon: Moon }].map(({ dark, label, icon }) => (
                  <button key={label} onClick={() => setIsDark(dark)} className={`flex items-center gap-2 rounded-xl border px-3 py-2 text-xs font-bold transition ${isDark === dark ? 'border-sky-300 bg-sky-50 text-sky-800' : 'border-[#e2e8f0] text-slate-600 hover:border-sky-300'}`}>{React.createElement(icon, { size: 15 })}{label}</button>
                ))}
              </div>
            </div>
            <div className="flex flex-col gap-4 py-5 sm:flex-row sm:items-center sm:justify-between">
              ...
              <select value={studentSettings.reminderTime} onChange={(event) => updateSetting('reminderTime', event.target.value)} className="rounded-xl border border-[#e2e8f0] bg-white px-3 py-2.5 text-xs font-bold outline-none focus:border-[#60a5fa] dark:border-white/10 dark:bg-[#151d2c]">
            ...
            <div className="flex flex-col gap-4 py-5 sm:flex-row sm:items-center sm:justify-between">
              ...
              <select value={studentSettings.language} onChange={(event) => updateSetting('language', event.target.value)} className="rounded-xl border border-[#e2e8f0] bg-white px-3 py-2.5 text-xs font-bold outline-none focus:border-[#60a5fa] dark:border-white/10 dark:bg-[#151d2c]">
  ...
  return (
    <div className={`${isDark ? 'soft-sky' : ''} student-dashboard campus-dashboard`}>
      ...
      {correctionRecord && <div className="fixed inset-0 z-[140] grid place-items-center bg-slate-950/55 p-4 backdrop-blur-sm" onMouseDown={(event) => event.target === event.currentTarget && setCorrectionRecord(null)}><form onSubmit={submitCorrection} className="w-full max-w-md rounded-2xl border border-[#e2e8f0] bg-white p-6 shadow-2xl dark:border-white/10 dark:bg-[#151d2c]"><div className="flex items-start justify-between gap-3"><div><p className={`text-[10px] font-bold uppercase tracking-wider ${muted}`}>Attendance correction</p><h3 className="mt-1 text-lg font-black">{correctionRecord.code} · {correctionRecord.date}</h3></div><button type="button" onClick={() => setCorrectionRecord(null)} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100 dark:hover:bg-white/5"><X size={18} /></button></div><div className="mt-5 rounded-xl bg-slate-50 p-4 text-xs dark:bg-white/5"><span className={muted}>Currently recorded</span><div className="mt-2 flex items-center justify-between"><b>{correctionRecord.subject}</b><Badge status={correctionRecord.status} /></div></div><div className="mt-5 space-y-4"><label className="block text-xs font-bold">Correct status<select value={correctionDraft.expectedStatus} onChange={(event) => setCorrectionDraft((value) => ({ ...value, expectedStatus: event.target.value }))} className="mt-2 w-full rounded-xl border border-[#e2e8f0] bg-white p-3 text-sm dark:border-white/10 dark:bg-[#151d2c]">{['Present','Late','Absent'].filter((status) => status !== correctionRecord.status).map((status) => <option key={status}>{status}</option>)}</select></label><label className="block text-xs font-bold">What went wrong?<textarea required rows="4" value={correctionDraft.reason} onChange={(event) => setCorrectionDraft((value) => ({ ...value, reason: event.target.value }))} placeholder="For example: I checked in at 07:56, but the record shows absent." className="mt-2 w-full resize-none rounded-xl border border-[#e2e8f0] bg-transparent p-3 text-sm outline-none focus:border-sky-400 dark:border-white/10" /></label></div><div className="mt-6 grid grid-cols-2 gap-3"><button type="button" onClick={() => setCorrectionRecord(null)} className="rounded-xl border border-[#e2e8f0] py-3 text-sm font-bold dark:border-white/10">Cancel</button><button type="submit" className="rounded-xl bg-sky-500 py-3 text-sm font-bold text-white hover:bg-sky-600">Submit correction</button></div></form></div>}
      ...
      <div className="campus-shell flex min-h-screen text-slate-800">
        ...
        <aside className={`campus-sidebar fixed inset-y-0 left-0 z-40 flex w-64 flex-col border-r border-white/80 transition-all duration-300 lg:sticky lg:top-0 lg:h-screen lg:translate-x-0 ${sidebarCollapsed ? 'lg:w-20' : 'lg:w-64'} ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          <button
            ...
            className="absolute -right-3 top-[68px] z-50 hidden h-6 w-6 items-center justify-center rounded-full border border-[#e2e8f0] bg-white text-slate-500 shadow-[0_3px_10px_rgba(15,23,42,0.16)] transition-all duration-200 hover:scale-110 hover:border-[#60a5fa] hover:bg-[#60a5fa] hover:text-white dark:border-white/15 dark:bg-[#151d2c] lg:flex"
          >
            {sidebarCollapsed ? <ChevronRight size={14} strokeWidth={2.5} /> : <ChevronLeft size={14} strokeWidth={2.5} />}
          </button>
          <div className="flex h-20 items-center justify-between border-b border-[#e2e8f0] px-4 dark:border-white/10">
          ...
          <div className="border-t border-[#e2e8f0] p-4 dark:border-white/10">
        ...
        <div className="min-w-0 flex-1">
          <main className={`mx-auto w-full ${activeView === 'schedule' ? 'max-w-none p-3 sm:p-4' : 'max-w-[1600px] p-4 sm:p-5'}`}>
            <div className="relative mb-5 flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <button aria-label="Open navigation" onClick={() => setSidebarOpen(true)} className="rounded-xl border border-[#e2e8f0] bg-white p-2.5 text-slate-500 shadow-sm dark:border-white/10 dark:bg-white/5 lg:hidden"><Menu size={20} /></button>
